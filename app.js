...
function loadContacts() {
  ...
  target.innerHTML = '';
  if (!list.length) { target.innerHTML = '<div class="text-muted">No contacts yet.</div>'; return; }
  list.forEach((c, i) => {
...
async function sendBulkList() {
  let recipients = parsedBulk.length ? parsedBulk.slice() : document.getElementById('bulkList').value.trim().split(/\r?\n/).map(s => s.trim()).filter(Boolean);
  if (!recipients.length) return alert('No recipients found');

  const fileEl = document.getElementById('bulkFile');
  const message = document.getElementById('bulkMessage').value.trim() || ' ';
  const hasFile = fileEl.files.length > 0;
  if (!hasFile && !message) return alert('Provide message or file');
  if (!confirm(`Send to ${recipients.length} recipients? ~${(recipients.length * RATE_DELAY_MS / 1000).toFixed(0)}s`)) return;
  ...
  for (let to of recipients) {
    const payload = { to, body: message, base64, filename };
    await postSendAndHandleResponse(payload, filename, message, to, { innerHTML: '' });
  ...
  el.innerHTML = list.map(s=>{
    ...
    return `<div class="p-2 border rounded mb-2 d-flex justify-content-between align-items-start">
      <div>
        ...
        <div class="small text-muted">${escapeHtml(s.recipients.join(', '))}</div>
        ${msg}
      </div>
...
function editSchedule(id){
  ...
  document.getElementById('scheduleRecipients').value = job.recipients.join('\n');
...
async function sendScheduleNow(){
  ...
  for(const s of list){
    ...
    for(const to of s.recipients){
      const payload = { to, body: s.message || ' ', base64: mf?.base64 || '', filename: mf?.filename || '' };
      await postSendAndHandleResponse(payload, mf?.filename || '', s.message || ' ', to, {innerHTML:''});
      await sleep(RATE_DELAY_MS);
    }
  }
